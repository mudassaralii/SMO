                ...
                for (var i = 0; i < tbl.length; i++) {
                    if (parseInt($("#role").html()) == 1) {
                        if (tbl[i].type == 'archive') {
                            tableArchive += '<tr>'
                            tableArchive += '<td></td>';
                            tableArchive += '<td><a href="uploads_mnt/' + tbl[i].orderid + '.pdf" target="_blank"><i class="fa fa-file-pdf" style="font-size:30px;color:red;"></i></a></td>';
                            ...
                            var row_style='';
                            var row_datesplit=tbl[i].end_date.split("/");
                            var row_date=new Date(row_datesplit[1]+"/"+row_datesplit[0]+"/"+row_datesplit[2]);
                            
                            if(curr_date>row_date & tbl[i].status_id =='2'){
                                row_style="style='background-color:pink'";
                            }else if(tbl[i].status_id =='1'){
                                row_style="style='background-color:lightgreen'";
                            }
                            tableFresh += '<tr '+row_style+' >';
                            tableFresh += '<td></td>';
                            tableFresh += '<td><a href="uploads_mnt/' + tbl[i].orderid + '.pdf" target="_blank"><i class="fa fa-file-pdf" style="font-size:30px;color:red;"></i></a></td>';
                            ...
                            if (tbl[i].status_id == '1') {
                              tableFresh += '<td><a href="javascript:void(0)"><i class="fas fa-pencil-alt"  style="font-size:20px;color:black;"></i></a></td>'
                            } else {
                              tableFresh += '<td><a id="toggleEdit-'+[i]+'" href="#" onclick="toggleEditFresh(' + [i] + ')"><i class="fas fa-pencil-alt"  style="font-size:20px;color:black;"></i></a></td>'
                            }

                            switch (tbl[i].status_id) {
                                case '1':
                                    tableFresh += '<td><select id="status-' + [i] + '" onchange="handleUpdate(this.id)" disabled><option value="1" selected>Completed</option><option value="2">In Progress</option><option value="3">Pending</option><option value="4">Order Placed</option></select></td>';
                                    break;
                                case '2':
                            ...
                            tableFresh += '<td><button id="Msgbtn_'+ tbl[i].orderid +'"onclick="onClickModal(\''+tbl[i].orderid+'\')" </button>' + tbl[i].orderid + '</td>';
                            // tableFresh += '<td><button id="btn_'+ tbl[i].orderid +'"onclick="promptCancellation(\''+ tbl[i].orderid +'\')" disabled>Cancellation Fwded</button></td>';
                           
                           
                            tableFresh += '<td>' + tbl[i].ordered_area + '</td>';
                        ...
                        if (tbl[i].type == 'archive') {
                            ...
                            tableFresh += '<td><button class="btnCancelOrder" id="btn_'+ tbl[i].orderid +'" onclick="promptCancellation(\''+ tbl[i].orderid +'\')">Cancel</button></td>';
                              ...
                              tableFresh += '<td><button class="btnCancelOrder disabled" id="btn_'+ tbl[i].orderid +'" onclick="promptCancellation(\''+ tbl[i].orderid +'\')" disabled>Cancellation Fwded</button></td>';
                            }
                            tableFresh += '<td>' + tbl[i].orderid + '</td>';
                            tableFresh += '<td>' + tbl[i].ordered_area + '</td>';
                            tableFresh += '<td>' + tbl[i].date + '</td>';
...
function onClickModal(orderid) {  
     $('#messageModal').modal('show');

     messageUpdate(orderid);
  }
...
function messageUpdate(id){
    ...
    $.ajax({
        type: "GET",
        url: "server_scripts/getmessages.php?Orderid="+id,
        success: function (data) {
            console.log("data"+data);
            // var msgarray = JSON.parse(data);
            // var messageString='';
            // msgarray.array.forEach(element => {
    ...
    messagepush(id);
    $('#messagetxtarea').val($('#messagetxt').val());

}

function messagepush(id){
    $.ajax({
        type: "POST",
        url: "server_scripts/pushMessage.php",
        data: {
            orderid: "id",
...
function updateRecord(val, id, type) {
  var area = parseInt($("#area-"+id).val());
  var status_id = $('#status-' + id + '').val();
  if (((area > 0 && type == 'fresh') || status_id !== "1") || type == 'archive') {
    ...
    $.ajax({
        ...
        success: function (data) {
          if (data == 'Success') {
            $("#toggleEdit-" + id).attr("href", "javascript:void(0)");
            $("#toggleEdit-" + id).prop("onclick", null);
    ...
    toastr.error("Area Less than 0 is not allowed");
  }
}
...
function paginateTable(val) {
    ...
    t = $('#' + val + '').DataTable({
        ...
        {
            "targets": (parseInt($("#role").html()) == 1)? (val.split('_')[1] == "archive") ? [0, 2, 14, 16, 18] : [0, 2, 3, 4] : null,
            "visible": false
        }
        ],
        "order": [
...
function checkCancellationRequests(){
    $.ajax({
        ...
        success: function (data) {
            ...
            tableCancel += "<th>Ordered Area</th>";
            tableCancel += "<th>Delivered Area</th>";
            tableCancel += "<th>Sensor</th>";
            ...
            for (let index = 0; index < data.length; index++) {
                ...
                tableCancel += '<td><a href="uploads_mnt/' + element.orderid + '.pdf" target="_blank"><i class="fa fa-file-pdf" style="font-size:30px;color:red;"></i></a></td>';
                ...
                tableCancel += "<td>" + element.ordered_area + "</td>";
                tableCancel += "<td>" + element.area + "</td>";
                tableCancel += "<td>" + element.sensor + "</td>";
