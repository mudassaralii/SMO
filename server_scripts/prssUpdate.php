...
include('db_pg.php');
	...
	$c1 = "select * from prss WHERE metadataid='" . $sceneid . "';";
	...
	while($edgec1=pg_fetch_assoc($queryc1)) {
		$date = new DateTime($edgec1['imageingstarttime']);
		$time = new DateTime($edgec1['imageingstarttime']);
		$feature = array
		(
			'data_id' => $edgec1['metadataid'],
			'platform_name' => $edgec1['satellitei'],
			//'platform_s_no' => $edgec1['platform_s_no'],
			'data_strip_type' => $edgec1['sensorid'],
